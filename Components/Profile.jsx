...
export default ({
  ...
  useEffect(() => {
    ...
    getShipmentsData.then(setCount);
    ...
    const fetchBalance = async () => {
      if (window.ethereum && currentUser) {
        try {
          ...
          const balance = await provider.getBalance(currentUser);
          const formattedBalance = parseFloat(ethers.utils.formatEther(balance)).toFixed(2); // Round to the hundredth place
